 ...
 * @author user
 */
public class SubjectPredecessorsController extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
        try {
            request.getRequestDispatcher("subjectpredecessors.jsp").forward(request, response);
        } catch (ServletException | IOException e) {
            System.out.println("error doGet SubjectPrede: " + e.getMessage());
        }
    }
        ...
        try {
            ...
            if (search != null) {
                ...
                if (sDAO.checkSubjectCode(code)) {
                    ArrayList<Prerequisite> listPrerequisiteSubjectByCode = pDAO.getListPrerequisiteSubjectByCode(code);
                    String subjectId = sDAO.getSubjectIdbySubjectCode(code).getId();
                    if (!pDAO.checkPrerequisiteSubjectBySubjectId(subjectId)) {
...
//                        HashMap<String, ArrayList<String>> subjects = new HashMap<String, ArrayList<String>>();
                        ...
                        //https://shareprogramming.net/xoa-tat-ca-cac-phan-tu-trung-lap-trong-java-list/
                        ...
                        for (String subject : arrayNew) {
                            List<Prerequisite> prerequisites = pDAO.getListPrerequisiteSubjectByCode(subject);
                            sb.append(subject).append(": ");
                            if (!prerequisites.isEmpty()) {
                                for (Prerequisite prerequisite : prerequisites) {
                                    String prerequisiteCode = sDAO.getSubjectCodeBySubjectId(prerequisite.getPrequisite_id()).getCode();
...
//                        testList.toArray(new String[0])
            ...
            System.out.println("error doPost SubjectPredecessors: " + e.getMessage());
        }
    }
    ...
    private void traverseSubjectPrerequisitesTree(String code, List<String> listPre) {
        ...
        while (!queue.isEmpty()) {
            ...
            List<Prerequisite> prerequisites = pDAO.getListPrerequisiteSubjectByCode(currentNode.code);
            for (Prerequisite prerequisite : prerequisites) {
                String preCode = sDAO.getSubjectCodeBySubjectId(prerequisite.getPrequisite_id()).getCode();
