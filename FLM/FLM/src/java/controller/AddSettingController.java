...
public class AddSettingController extends HttpServlet {
    ...
    throws ServletException, IOException {
        SettingDAO st = new SettingDAO();
            ...
            if (st.checkSettingNameByName(name)) {
                request.setAttribute("mess", "Name already exists!");
                request.getRequestDispatcher("addsetting.jsp").forward(request, response);
                return;
            }
            if (request.getParameter("type") != null) {
                if (st.checkSettingTypeByType(type)) {
                    request.setAttribute("mess", "Type already exists!");
                    request.getRequestDispatcher("addsetting.jsp").forward(request, response);
                    return;
                }
            }
            Setting s = new Setting(id, name, type, value, displayorder, description);
            if (request.getParameter("add") != null) {
                boolean success = st.addNewSetting(s);
            }
